...
public:
    ...
    static void isActiveCccCfgChanged(const struct bt_gatt_attr *attr, uint16_t value)
    {
        ...
        printk("Anim %d isActive attr %p ccc %u\n", T::kBtServiceIdNum, attr, value);
...
protected:
    static void btNotifyIfEnabled()
    {
        ...
        {
            printk("Anim %d isActive notifying %p on value change\n", T::kBtServiceIdNum, T::getInstance()->activeAttr_);
            int ret = bt_gatt_notify(NULL, T::getInstance()->activeAttr_, &T::getInstance()->active_, sizeof(T::getInstance()->active_));
            if (ret != 0)
            {
                printk("Anim %d isActive notification failed: %d\n", T::kBtServiceIdNum, ret);
            }
        }
        else
        {
            printk("Anim %d isActive notification not enabled, not notifying\n", T::kBtServiceIdNum);
        }
    }
