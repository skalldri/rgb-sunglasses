...
#define TPS25750_INT_BIT_LIST                                                                      \
    ...
    /* Byte 5 */                                                                                   \
    /* 7 Reserved */                                                                               \
    TPS25750_INT_BIT(ErrorUnableToSource, 5, 6) /* The Source was unable to increase the voltage   \
                                                   to the negotiated voltage of the contract. */   \
    /* 5:4 Reserved */                                                                             \
    TPS25750_INT_BIT(PlugEarlyNotification, 5,                                                     \
                     3) /* A connection has been detected but not debounced. */                    \
    TPS25750_INT_BIT(SnkTransitionComplete, 5,                                                     \
                     2) /* This event only occurs when in source mode (PD_STATUS.PresentPDRole =   \
                           1b). It occurs tSrcTransition (ms) after sending an Accept message to a \
                           Request message, just before sending the PS_RDY message. */             \
    /* 1 Reserved */                                                                               \
    /* 0 Reserved */                                                                               \
    /* Byte 4 */                                                                                   \
    TPS25750_INT_BIT(ErrorMessageData, 4, 7) /* An erroneous message was received. */              \
    TPS25750_INT_BIT(ErrorProtocolError, 4,                                                        \
                     6) /* An unexpected message was received from the partner device. */          \
    /* 5 Reserved */                                                                               \
    TPS25750_INT_BIT(ErrorMissingGetCapMessage, 4,                                                 \
                     4) /* The partner device did not respond to the Get_Sink_Cap or               \
                           Get_Source_Cap message that was sent. */                                \
    TPS25750_INT_BIT(ErrorPowerEventOccurred, 4,                                                   \
                     3) /* An OVP, or ILIM event occurred on VBUS. Or a TSD event occurred. */     \
    TPS25750_INT_BIT(ErrorCanProvideVoltageOrCurrentLater, 4,                                      \
                     2) /* The USB PD Source can provide acceptable voltage and current, but not   \
                           at the present time. A "wait" message was sent or received. */          \
    TPS25750_INT_BIT(                                                                              \
        ErrorCannotProvideVoltageOrCurrent, 4,                                                     \
        1) /* The USB PD Source cannot provide an acceptable voltage and/or current. A Reject      \
              message was sent to the Sink or a Capability Mismatch was received from the Sink. */ \
    TPS25750_INT_BIT(ErrorDeviceIncompatible, 4,                                                   \
                     0) /* When set to 1, a USB PD device with an incompatible specification       \
                           version was connected. Or the partner device is not USB PD capable. */  \
    /* Byte 3 */                                                                                   \
    /* 7 Reserved */                                                                               \
    TPS25750_INT_BIT(CMDComplete, 3,                                                               \
                     6) /* Set whenever a non-zero value in CMD register is set to zero or !CMD */ \
    /* 5 Reserved */                                                                               \
    /* 4 Reserved */                                                                               \
    TPS25750_INT_BIT(PDStatusUpdate, 3,                                                            \
                     3) /* Set whenever contents of PD_STATUS register (0x40) change. */           \
    TPS25750_INT_BIT(StatusUpdate, 3,                                                              \
                     2) /* Set whenever contents of STATUS register (0x1A) change. */              \
    /* 1 Reserved */                                                                               \
    TPS25750_INT_BIT(PowerStatusUpdate, 3,                                                         \
                     0) /* Set whenever contents of POWER_STATUS register (0x3F) change. */        \
    /* Byte 2 */                                                                                   \
    TPS25750_INT_BIT(PPswitchChanged, 2,                                                           \
                     7) /* Set whenever contents of POWER_PATH_STATUS register (0x26) changes. */  \
    /* 6 Reserved */                                                                               \
    TPS25750_INT_BIT(                                                                              \
        UsbHostPresentNoLonger, 2,                                                                 \
        5) /* Set when STATUS.UsbHostPresent transitions to anything other than 11b. */            \
    TPS25750_INT_BIT(UsbHostPresent, 2,                                                            \
                     4) /* Set when STATUS.UsbHostPresent transitions to 11b. */                   \
    /* 3 Reserved */                                                                               \
    ...
    /* Byte 1 */                                                                                   \
    /* 7 Reserved */                                                                               \
    TPS25750_INT_BIT(SourceCapMsgRcvd, 1, 6) /* This is asserted when a Source Capabilities        \
                                                message is received from the Port Partner. */      \
    TPS25750_INT_BIT(                                                                              \
        NewContractAsProv, 1,                                                                      \
        5) /* An RDO from the far-end device has been accepted and the PD Controller is a Source.  \
              This is asserted after the PS_RDY message has been sent. See ACTIVE_CONTRACT_PDO     \
              register (0x34) and ACTIVE_CONTRACT_RDO register (0x35) for details. */              \
    TPS25750_INT_BIT(NewContractAsCons, 1,                                                         \
                     4) /* Far-end source has accepted an RDO sent by the PD Controller as a Sink. \
                           See ACTIVE_CONTRACT_PDO register (0x34) and ACTIVE_CONTRACT_RDO         \
                           register (0x35) for details. */                                         \
    /* 3:0 Reserved */                                                                             \
    /* Byte 0*/                                                                                    \
    /* 7:6 Reserved*/                                                                              \
    TPS25750_INT_BIT(DRSwapComplete, 0,                                                            \
                     5) /* A Data Role swap has completed. See STATUS register (0x1A) and          \
                           PD_STATUS register (0x40) for port state.*/                             \
    TPS25750_INT_BIT(PRSwapComplete, 0,                                                            \
                     4) /* A Power role swap has completed. See STATUS register (0x1A) and         \
                           PD_STATUS register (0x40) for port state.*/                             \
    TPS25750_INT_BIT(                                                                              \
        PlugInsertOrRemoval, 0,                                                                    \
        3) /* USB Plug Status has Changed. See Status register for more plug details.*/            \
    /* 2 Reserved*/                                                                                \
    TPS25750_INT_BIT(PDHardReset, 0, 1) /* A PD Hard Reset has been performed. See                 \
...
#define TPS25750_INT_BIT(_name, _byte, _bit) bool _name;
