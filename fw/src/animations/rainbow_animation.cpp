...
void RainbowAnimation::tick(AnimationRenderer &renderer, size_t timeSinceLastTickMs) {
    ...
    for (size_t x = 0; x < renderer.displayWidth(); x++) {
        size_t currentRainbowColor =
            ((currentRainbowStep + x) / rainbowColorWidth) % numRainbowColors;
        ...
        float red = ((1.0f - blendPercent) * ((float)rainbowColors[currentRainbowColor].r)) +
                    (blendPercent * ((float)rainbowColors[nextRainbowColor].r));
        float green = ((1.0f - blendPercent) * ((float)rainbowColors[currentRainbowColor].g)) +
                      (blendPercent * ((float)rainbowColors[nextRainbowColor].g));
        float blue = ((1.0f - blendPercent) * ((float)rainbowColors[currentRainbowColor].b)) +
                     (blendPercent * ((float)rainbowColors[nextRainbowColor].b));
