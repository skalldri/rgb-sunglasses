...
static constexpr bt_uuid_128 kMcubootInfoServiceUuid = BT_UUID_INIT_128(
    BT_UUID_128_ENCODE(0x12345678, 0x1234, 0x5678, 3, 0x56789abc0000));
...
static void read_and_format_version(char *buf, size_t bufsz) {
    ...
    if (rc == (int)sizeof(ver)) {
        snprintf(buf, bufsz, "%u.%u.%u+%u",
                 ver.iv_major, ver.iv_minor, ver.iv_revision, ver.iv_build_num);
    } else {
