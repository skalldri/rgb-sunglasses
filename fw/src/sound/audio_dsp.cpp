...
/* Display bucket boundaries live in audio_dsp.h (audio_display_bucket_start/end) so the
 * FFT Bars mapping's octave table can be tested against them. */
...
void audio_dsp_process(const int16_t *pcm, uint32_t seq, struct audio_analysis_result *out) {
    ...
    for (int b = 0; b < NUM_DISPLAY_BUCKETS; b++) {
        float32_t energy = 0.0f;
        for (int k = audio_display_bucket_start[b]; k <= audio_display_bucket_end[b]; k++) {
            energy += s_magnitude[k - 1]; /* s_magnitude[0] = bin 1 */
        }
        energy /= (float32_t)(audio_display_bucket_end[b] - audio_display_bucket_start[b] + 1);
