...
| Audio features | The **real** `fw/src/sound/audio_dsp.cpp` + SDK CMSIS-DSP compiled to WASM. Verified vs the native_sim replay harness on a click track: max scale-relative float difference 5.5e-5, **0/250 beat decisions differ** (`fw/tools/beat_lab/compare_sim.py`, gated in CI). |
...
| Symbol resolution | wasi-sdk links libc/libm statically — `sinf()` **works in the sim**. The zero-import gate rejects wasm *imports*, not statically-satisfiable libc calls. | The device links against ~10 Zephyr-exported symbols only; `sinf()` **fails at llext load time**. | This is why the ARM build stays mandatory. Keep using integer math (see plasma's `wave8()`). |
...
| Float arithmetic | wasm IEEE-754 (deterministic across platforms/browsers). libm (`cosf` in the Hann window, `log1pf` in flux) is wasi-libc. | ARM VFMA contraction + newlib libm. | Audio features differ from the device in late bits (see the measured 5.5e-5 above vs native_sim; device-vs-native_sim has the same class of difference — `fw/tools/beat_lab/compare.py`). Beat decisions on borderline frames can flip. |
