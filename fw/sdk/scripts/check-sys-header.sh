...
#
#   check-sys-header.sh [--cc <compiler>] [--lang c|c++]
#                       [--include-dir <dir>] [--allowed <file>]
#
# --lang c++ emits the probe as a .cpp and adds -std=c++23. Run BOTH languages:
# the header is consumed from C++ at least as often as from C (the C++ wrapper
# path in rgbx_animation.h is what getting-started.md leads with), and the two
# differ in ways that matter here — a dropped `extern "C"` block, or a future
# declaration colliding with the noexcept/overload set a C++ libc header
# attaches, is invisible to a C-only probe.
#
...
#   extern chk_t_printk *const chk_printk;
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        *)
            echo "usage: check-sys-header.sh [--cc <compiler>] [--lang c|c++] [--include-dir <dir>] [--allowed <file>]" >&2
            exit 2
            ;;
    esac
done
...
case "$LANG_MODE" in
    c) SRC_EXT="c"; LANG_FLAGS=() ;;
    c++) SRC_EXT="cpp"; LANG_FLAGS=(-std=c++23) ;;
    *) echo "error: --lang must be 'c' or 'c++', got '$LANG_MODE'" >&2; exit 2 ;;
esac
...
while IFS='|' read -r sym ret params; do
    [ -n "$sym" ] || continue
    # The preceding `extern` declaration is load-bearing, not noise. A
    # file-scope const has EXTERNAL linkage in C but INTERNAL linkage in C++,
    # so without it the C++ probe trips -Wunused-const-variable (measured:
    # wasi clang++ errors, g++ does not) and the gate fails for a reason that
    # has nothing to do with the prototypes under test.
    printf 'typedef %s chk_t_%s%s;\nextern chk_t_%s *const chk_%s;\nchk_t_%s *const chk_%s = %s;\n' \
        "$ret" "$sym" "$params" "$sym" "$sym" "$sym" "$sym" "$sym" >> "$src"
    checked=$((checked + 1))
done <<<"$PROTOTYPES"

if ! "$CC_BIN" "${LANG_FLAGS[@]}" -c "$src" -o "$tmp/check.o" -I "$INCLUDE_DIR" -Wall -Wextra -Werror 2>"$tmp/err"; then
...
echo "OK [$LANG_MODE]: <rgbx/rgbx_sys.h> declares all $checked checkable allowed symbols correctly ($n_excluded __aeabi_* helpers excluded by design)"
