...
- Debug shell: `ext list` / `ext select <slot>` / `ext param <slot> <idx> [<value>]` (type-aware: bools 0/1, strings as text) / `ext stats` (per-tick **cpu** and **wall** min/avg/max µs, printed separately — see the budget bullet below). The hello kitchen-sink demo doubles as the recovery test (`Crash`/`Hang` bool params).
...
- **LIST** (cmd 0, read, paginated) returns the union of the fenced directory's disk contents and the boot slot registry by FILE name, so "uploaded since boot" and "deleted since boot but still loaded" are first-class states the app renders directly.
