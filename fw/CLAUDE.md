...
**Threads still blocked from conversion** (missing syscall coverage in NCS v3.1.1, confirmed by grepping `__syscall` across the actual headers this project uses, not inferred): the entire Bluetooth host stack, the USB device_next stack, mcumgr, the settings subsystem, the filesystem API (`fs_*`), `flash_area_*`, `dmic_*`, and the WS2812 `led_strip_update_rgb()` driver call all have **zero** `__syscall` wrappers. `bt_thread`, `audio_dsp_thread` (also does raw MMIO register pokes for AGC gain), the MCUboot updater work queue, and the settings-save work queue should stay kernel-mode indefinitely barring upstream NCS syscall additions (per the "never touch the NCS SDK" rule).

**No further whole-thread conversions are planned — the LLEXT sandboxing plan is the "extension host thread" design in issue #85 (decided 2026-07-03, superseding an earlier plan to convert the LED/pattern threads).** Only third-party extension code needs sandboxing, so one dedicated K_USER host thread runs extension `init()`/`setActive()`/`tick()`; `pattern_controller_thread`, `led_display_thread`, and `status_led_thread` all stay kernel-mode, built-in animations keep running in kernel mode (so `GlimPlayerAnimation`'s per-frame `fs_read()` stays legal), and no custom `led_strip_update_rgb` syscall is needed. Interface: snapshot-in / copy-out per tick — kernel side snapshots inputs (BT params, IMU, audio) into a shared partition, the extension renders into a scratch framebuffer in its own partition (never the real one — the ~1.4KB/frame memcpy is the price for the extension domain needing zero access to `led_controller` internals), and the pattern controller enforces a per-tick deadline with `k_thread_abort()` + thread-recreate + fallback-animation recovery. The extension-side ABI (whether `BaseAnimation`/`AnimationRenderer` crosses the boundary as-is or the EDK exports a flatter C ABI) is deliberately unresolved until prototyping starts — see issue #85 for the full design.
