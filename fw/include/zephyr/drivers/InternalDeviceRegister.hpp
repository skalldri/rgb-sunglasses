...
class RegisterField {
    static_assert(bitWidth > 0 && bitWidth <= 32, "bitWidth must be 1-32");
    static_assert(firstBit >= 0 && firstBit < 31, "firstBit must be 0-31");
    static_assert((firstBit + bitWidth) <= 32, "firstBit + bitWidth must be <= 32");

   public:
    /**
     * @brief Get the register field value from the provided uint32_t based on the firstBit and
     * bitWidth settings
     *
     * @param reg the full value of the register
     * @return uint32_t the value of this field in the register
     */
    static uint32_t getValue(uint32_t reg) { return (reg >> firstBit) & ((1 << bitWidth) - 1); }
...
class I2CDeviceRegister : public DeviceRegister<name, registerWidth, U, Us...> {
   public:
     ...
     */
    I2CDeviceRegister(const struct i2c_dt_spec *i2c) : i2c_(i2c) { read(); }

    /**
     * @brief Destroy the I2CDeviceRegister object. If the object contains writes that have not been
     * flushed, write them now.
     *
     */
    ~I2CDeviceRegister() {
        if (dirty_) {
     ...
     */
    int read() {
        int ret = i2c_burst_read_dt(i2c_, reg, reinterpret_cast<uint8_t *>(&this->storage_),
                                    this->regWidth_ / 8);
