...
static int net_core_sim_flash_read(const struct device *dev, const off_t offset, void *data,
    ...
    if (offset == 0 && len == sizeof(struct image_header)) {
        ...
        // There will be 1 TLV, which is composed of an image_tlv header and the data payload, a SHA
        // hash
    ...
    // bytes as a `struct image_tlv`. Handle that case by producing a fake TLV that
        ...
        tlv->it_len = IMAGE_SHA_LEN -
                      sizeof(struct image_tlv);  // This is how many bytes we need to fast-forward
        return 0;
    }
...
static int net_core_sim_flash_write(const struct device *dev, const off_t offset, const void *data,
                                    const size_t len) {
    ARG_UNUSED(dev);
    ARG_UNUSED(offset);
    ARG_UNUSED(data);
...
static const struct flash_parameters *net_core_sim_flash_get_parameters(const struct device *dev) {
    ARG_UNUSED(dev);
...
DEVICE_DT_INST_DEFINE(0, net_core_sim_flash_init, NULL, NULL, NULL, POST_KERNEL,
                      CONFIG_FLASH_INIT_PRIORITY, &net_core_sim_flash_api);