export const meta = {
  ...
  description: "Opus 5 code review. Workflow-backed code review — one finder per correctness angle plus one finder covering all cleanup angles, an independent verifier for every distinct (file, line) location across the pooled candidates, then a ranked, capped findings report.",
...
const scope = await agent(
  ...
  { model: "opus", label: "scope", schema: SCOPE_SCHEMA }
)
if (!scope) {
...
async function verifyGroups(candidates) {
  ...
  const out = await parallel(groups.map(g => async () => {
    ...
    const r = await agent(GROUP_VERIFIER_PROMPT(g), { model: "opus", label: "verify:" + short + "(" + g.length + ")", phase: "Verify", schema: GROUP_VERDICT_SCHEMA })
    if (!r) return []
    const byIdx = {}
...
const finderOuts = await parallel(FINDERS.map(f => () =>
  agent(FINDER_PROMPT(f), { model: "opus", label: f.label, phase: "Find", schema: CANDIDATES_SCHEMA }).then(r => {
    if (!r) return []
...
if (P.sweep) {
  ...
  const sweep = await agent(
    ...
    { model: "opus", label: "sweep", phase: "Sweep", schema: CANDIDATES_SCHEMA }
  )
...
const report = await agent(
  ...
  { model: "opus", label: "synthesize", schema: REPORT_SCHEMA }
)
