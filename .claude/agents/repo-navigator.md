...
| Sandboxed .llext extensions | host side `fw/src/extensions/` (anchor: `extension_host`), ABI headers `fw/include/rgbx/` (`rgbx_api.h`, `rgbx_animation.h`), sample extensions + build `fw/extensions/` (`hello/`, `cpptest/`, `build.sh`, `README.md`) |
