...
Work top to bottom. Every section has a grep anchor into the **Rainbow** exemplar —
open the anchor's match, copy its shape, rename. `<Name>` = PascalCase enum name,
`<name>` = snake_case file/shell name, `<NAME>` = SCREAMING_SNAKE Kconfig name.
...
- **Pattern B (audio/IMU/button source, possibly + BLE params)** — individual
  ...
  providing subsystem** (`..._bind_default_sound_dependencies()` defined in
  `fw/src/sound/animation_adapters/audio_animations_sound.cpp`,
  `..._bind_default_bt_dependencies()` in the `_bt.cpp` adapter, etc.).
...
Add `<Name> = <next-integer>,` immediately before the `Count` sentinel comment.
`Count` must stay last — `fw/src/extensions/extension_limits.h` static_asserts that
extension IDs (0x40+) stay above `Animation::Count`.
...
- `#include <animations/animation.h>` and `<animations/animation_parameter_source.h>`
  (plus `<animations/animation_audio_source.h>` / `animation_imu_source.h` for Pattern B)
- Pattern A: `class <Name>AnimationDependencies` with
  `const AnimationUint32ParameterSource&` members initialized by constructor
- `class <Name>Animation : public BaseAnimationTemplate<<Name>Animation, Animation::<Name>>`
  with `setDependencies(...)` (or per-source setters), `void init() override;`,
  `void tick(AnimationRenderer&, size_t timeSinceLastTickMs) override;`
- private `const <Name>AnimationDependencies *deps_ = nullptr;`
- free-function declaration(s): `void <name>_animation_bind_default_dependencies();`
  (Pattern B: one per subsystem, e.g. `..._bind_default_sound_dependencies();` AND
...
- NO `bluetooth/` includes, no `BtGatt*` symbols. Verify when done:
  `grep -n "bluetooth\|BtGatt" fw/src/animations/<name>_animation.cpp` → no hits.
- First line of `tick()` guards unbound deps:
  - Pattern A: `__ASSERT(deps_, "<Name>Animation::tick before setDependencies");`
  - Pattern B: `if (!audioSource_) { /* blank display, return */ }` — copy the
    blank-and-return loop from `beat_animation.cpp` or `tilt_animation.cpp`.
- Pattern B: call `source->update()` exactly ONCE at the start of each tick (the
  interface headers mandate this), then use the getters. Never touch the IMU/audio
  msgqs directly.
- Obey the RENDERING RULES box in SKILL.md (255-level colors,
  `renderer.displayWidth()/Height()`, no off-display writes, nose cutout).
...
3. **One `BtGattPersistentCharacteristic` per parameter**:
   ...
   — the first template arg is the persisted settings key (stable literal, never
   derived), the label becomes the app's auto-rendered control name.
...
6. **Server + link-time registration**:
   ...
   ⚠️ Argument order is positional and assigns characteristic UUIDs — once shipped,
   NEVER reorder. Internals: /add-gatt-characteristic.
...
Pattern B: the audio/IMU/button bind functions are NOT here — they live in the
subsystem adapter (`grep -n "bind_default" fw/src/sound/animation_adapters/audio_animations_sound.cpp fw/src/imu/animation_adapters/imu_animations_imu.cpp fw/src/buttons/animation_adapters/button_animation_source.cpp`);
add yours next to the existing ones there.
...
`default y` ONLY if it fits on both boards (DK FLASH is nearly full — see SKILL.md
step 8). Otherwise omit the default and enable per-board (step 8 below).
Never reuse another subsystem's symbol (fw/CLAUDE.md rule).
...
Pattern B additionally: extend the shared-adapter gate so the subsystem adapter
compiles when only your animation enables it. **Each subsystem has its own gate —
extend the one for YOUR subsystem** (extending the wrong one leaves your bind
function uncompiled: a link error far from the edit):
...
- **Buttons** — `button_animation_source.cpp` is compiled unconditionally; no gate
  to extend.
...
(4 hits: the using-alias, factory, activator, and registration guards. Rainbow's
`#include` is unconditional; guard yours like matrix_code's —
...
3. **Registration block** — this exact shape, ORDER AND RETURN CHECKS MANDATORY:
   ...
   `register_is_active` before `register` returns `-ENOENT`
   (`grep -n ENOENT fw/src/animations/animation_registry.cpp`). Background: an
   ignored return in this area once silently killed the Is Active notify path
   (caught in PR #89 review) — no build or test gate catches it, only the checks.
...
Add `CONFIG_ANIMATION_<NAME>=y` (skip if the Kconfig entry is `default y`).
The file is the FLAT `fw/boards/<board_target>.conf` — a conf placed inside
`fw/boards/others/<board>/` is silently ignored. Shared-on-both-boards settings go
in `fw/prj.conf` instead (anchor: `grep -n CONFIG_ANIMATION_BEAT fw/prj.conf`).
After building, confirm the symbol in
`fw/build/fw/zephyr/include/generated/zephyr/autoconf.h`.
...
Full recipe, pitfalls, and the fake-renderer/fake-source patterns: **/add-fw-test**.
Key facts: no `CONFIG_BT` needed (old requirement, gone); testcase name must be
dotted (`animations.<name>_animation_di`); compile the animation +
`animation_registry.cpp` + `animation_base.cpp` only. Not every legacy animation has
a suite (matrix_code doesn't) — every new one must.
...
- **Spot A — `anim set` dict**: `SHELL_SUBCMD_DICT_SET_CREATE(sub_anim_set, ...)`.
  ...
  list. Kconfig-conditional animations: use the macro pattern — anchor
...
Missing either compiles clean and silently breaks `anim set <name>` / `anim get` —
and everything layered on them (serial-MCP tools, /flash-and-verify checks).
...
If the animation should be drivable via `mcp__serial__rgb_sunglasses_set_animation`,
add its shell name (same string as step 10) to the `SETTABLE_ANIMATIONS` list.
Restart the serial MCP server to pick it up.
...
1. **DI suite alone** (seconds):
   ...
   (scratch `--outdir` per /test-fw's targeted-run rule — omitting it leaves a
   non-gitignored `twister-out/` at the repo root)
2. **Both boards link**: /build-proto0 then /build-dk. Watch the DK FLASH% in the
   build output — if the DK overflows, gate the feature off the DK via the board
   conf / Kconfig default (see /rom-ram-budget). Confirm `CONFIG_ANIMATION_<NAME>=y`
   landed in `fw/build/fw/zephyr/include/generated/zephyr/autoconf.h`.
3. **Full gate**: /submit-pr (runs both builds + the whole test suite + coverage).
   On-device behavior (does it actually draw?) is /flash-and-verify — hardware lock
