...
| `fw/extensions/**`, `fw/include/rgbx/**`, `fw/src/extensions/**` | Simulator smoke: `fw/sim/rgbx-sim run <ext> --scenario silence` + one input-relevant scenario (metronome-120 / head-tilt / buttons-tour — proves the tick logic actually renders and reacts, no proto0 build needed) **and** compile every extension with `fw/extensions/build.sh` (requires an existing proto0 build in `fw/build` — run `/build-proto0` first; proves ARM linkage the sim cannot — `fw/sim/PARITY.md`) + run the `extensions.manifest` Twister suite (`fw/tests/extensions/manifest`); `/add-extension` covers all three | On-device runtime of a `.llext` (llext load, relocation, MPU, timing budgets) is **hardware-only** — sim + cross-compile prove logic and linkage, not loading or timing; install and observe per `/add-extension` + `/flash-and-verify` |
