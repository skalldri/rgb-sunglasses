...
| `fw/extensions/**`, `fw/include/rgbx/**` | Simulator smoke: `fw/sim/rgbx-sim run <ext> --scenario silence` + one input-relevant scenario (metronome-120 / head-tilt / buttons-tour — proves the EXTENSION's tick logic renders and reacts, no proto0 build needed) **and** compile every extension with `fw/extensions/build.sh` (requires an existing proto0 build in `fw/build` — run `/build-proto0` first; proves ARM linkage the sim cannot — `fw/sim/PARITY.md`) + run the `extensions.manifest` Twister suite; `/add-extension` covers all three | On-device runtime of a `.llext` (llext load, relocation, MPU, timing budgets) is **hardware-only** — sim + cross-compile prove logic and linkage, not loading or timing; install and observe per `/add-extension` + `/flash-and-verify` |
| `fw/src/extensions/**` (the extension HOST) | The relevant Twister suites (`extensions.*`, `bluetooth.extension_bt`) — note the sim does NOT verify host changes: `fw/sim/core/` is a hand-ported TS replica of `extension_host.cpp` semantics, so sim smoke passing proves nothing about modified host C++. If the change alters tick/fault/param semantics, update the TS port + `fw/sim/PARITY.md` to match in the same PR | Host runtime behavior is **hardware-only** — a clean cross-compile and green sim prove nothing about the real sandbox thread, MPU domains, or budget accounting; `/flash-and-verify` + `ext` shell verification required |
| `fw/sim/**` | `cd fw/sim && npm test` (builds TS + runs unit/integration suites — run `fw/sim/build-extensions.sh` first so the integration tests don't skip) + `rgbx-sim run` both example extensions through one scenario each + `rgbx-sim compare` (golden digests); `sim-ci.yml` runs the same on the PR. If `fw/src/sound/audio_dsp*` also changed, run the DSP parity check (`fw/sim/README.md` "Audio-DSP parity") | n/a — the simulator itself never touches hardware |
